...
flow_salvage <- left_join(flow_vernalis_daily, salvage_daily) %>%
  mutate(Salvage = replace(Salvage, is.na(Salvage), 0),
          threshold = ifelse(Flow >= 8000, "VNS >= 8000", "VNS < 8000"),
...
```{r, echo = FALSE, fig.cap= "Salvage as a Function of Vernalis Flow", width = 8, height = 6}
ggplot(flow_salvage) + 
  geom_point(aes(Date, Flow), size = 0.8, color = "gray30") +
  geom_point(aes(Date, salvageSc, color = threshold), size = 0.8) + 
  geom_hline(yintercept = 8000, color = "red") + 
  scale_x_date(date_breaks = "1 year", date_labels = "%Y", expand = c(0,0)) + 
  scale_color_manual(values = c("slategray2", "goldenrod2"))+
...
tab <- incidents %>%
  select(WY, Salvage) %>%
  group_by(WY) %>%
  summarize(AnnualSalvage = sum(Salvage, na.rm = TRUE)) %>%
  ungroup() %>%
  mutate(WY = factor(WY)) 

flextable(tab) %>%
  set_caption("Annual Salvage During Periods Where VNS >= 8000 cfs")
```
