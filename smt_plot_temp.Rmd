...
temp_daily <- temp_sum %>%
  ...
  mutate(fWY = factor(wy),
         index = 1:n()) %>%
  mutate(station_name = case_when(station == "BLP" ~ "SJR Blind Point",
                                  station == "HLT" ~ "Middle River Holt",
                                  station == "HOL" ~ "Holland Cut")) %>%
  ...
  filter(!is.na(wytype), (month >1 & month < 6), wy > 2009 & wy< 2020) %>%
   mutate(wytype = factor(wytype, levels = c("W", "AN", "BN","D", "C")),
          fWY = factor(wy),
          over12 = ifelse(meanTemp > 12, 20, 0))
...
plot_temp <- ggplot2::ggplot()+
  ...
  ggplot2::geom_hline(yintercept = 12)+
   geom_point(data = firsttemp, aes(x = date2, y = 12, color = station), size = 2, shape = 5) + facet_wrap(~fWY, strip.position = "left", nrow = 10)+
